...
namespace ExcelComponent.Excel.Ole2
{
	...
	public class Directory
	{
		...
		{
			if (ExcelComponent.Excel.ByteUtil.Bytes.AreEqual(streamName, RootName))
			{
				return new Bytes(new byte[]{0x05});
			}
			else if (ExcelComponent.Excel.ByteUtil.Bytes.AreEqual(streamName, Biff8Workbook))
			{
				return new Bytes(new byte[] { 0x02 });
			}
			else if (ExcelComponent.Excel.ByteUtil.Bytes.AreEqual(streamName, StreamNameSumaryInformation))
			{
				return new Bytes(new byte[] { 0x02 });
			}
			else if (ExcelComponent.Excel.ByteUtil.Bytes.AreEqual(streamName, StreamNameDocumentSummaryInformation))
			{
				return new Bytes(new byte[] { 0x02 });
			}
			else
		...
		private static int HackDirectoryDID(byte[] streamName, string didType)
		{
			...
			else if (ExcelComponent.Excel.ByteUtil.Bytes.AreEqual(streamName, new byte[] { //BIFF8 '&05HDocumentSummaryInformation'
			    0x05, 0x00, 0x44, 0x00, 0x6F, 0x00, 0x63, 0x00, 0x75, 0x00, 0x6D, 0x00, 0x65, 0x00, 0x6E, 0x00,
				0x74, 0x00, 0x53, 0x00, 0x75, 0x00, 0x6D, 0x00, 0x6D, 0x00, 0x61, 0x00, 0x72, 0x00, 0x79, 0x00,
				0x49, 0x00, 0x6E, 0x00, 0x66, 0x00, 0x6F, 0x00, 0x72, 0x00, 0x6D, 0x00, 0x61, 0x00, 0x74, 0x00,
				0x69, 0x00, 0x6F, 0x00, 0x6E, 0x00, 0x00, 0x00}))
