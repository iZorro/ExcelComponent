...
namespace ExcelComponent.Excel.Ole2
{
    /// <summary>
	...
	public class Ole2Document
	{
		...
		public byte[] DocFileID
		{
			...
			{
				if (value.Length != 8)
                    throw new Exception("value must be 8 bytes in length");
		...
		public byte[] DocUID
		{
			...
			{
				if (value.Length != 16)
                    throw new Exception("value must be 16 bytes in length");
		...
		public byte[] FileFormatRevision
		{
			...
			{
				if (value.Length != 2)
                    throw new Exception("value must be 2 bytes in length");
		...
		public bool IsLittleEndian
		{
            ...
            set { if (value == false) throw new Exception("Big Endian not currently supported"); _isLittleEndian = value; }
		}
		...
		public ushort SectorSize
		{
			...
			{
				if (value < 7)
                    throw new Exception("value must be >= 7");
		...
		public ushort ShortSectorSize
		{
			...
			{
				if (value > SectorSize)
                    throw new Exception(string.Format("value must be <= SectorSize {0}", SectorSize));
		...
		public void Load(System.IO.Stream stream)
		{
			...
			if (!isLE)
                throw new Exception("File is not Little-Endian");
...
//		    m = (int) (Math.Pow(2, sectorSize) / 4);
            ...
            for (i = 0; i < dir.Count; i++)
            {
                ...
                if (ExcelComponent.Excel.ByteUtil.Bytes.AreEqual(docStreamName, Directory.RootName))
                    overwrite = true;
                Bytes docStream =
