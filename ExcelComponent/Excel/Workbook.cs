...
namespace ExcelComponent.Excel
{
	...
	public class Workbook
	{
		...
		private readonly CellFormats _xfs;
        ...
        internal Workbook(XlsDocument doc)
		{
			...
			_xfs = new CellFormats(_doc, this);
        ...
        private void ReadBytes(Bytes bytes, BytesReadCallback bytesReadCallback)
        {
            ...
            if (bytes.Length == 0)
                throw new Exception("bytes can't be zero-length ");
            ...
            SortedList<ushort, CellFormat> xfs = new SortedList<ushort, CellFormat>();
            ...
            for (index = 0; index < xfRecords.Count; index++)
            {
                ...
                xfIdLookups[index] = this.XFs.Add(new CellFormat(_doc, record.Data, font, format));
            }
