...
using System.IO;
using System.Text;
using ExcelComponent.Excel.Ole2;
using ExcelComponent.Excel.Ole2.Metadata;
using Bytes = ExcelComponent.Excel.ByteUtil.Bytes;
using Directory = ExcelComponent.Excel.Ole2.Directory;
using MetadataStream = ExcelComponent.Excel.Ole2.Metadata.MetadataStream;
...
namespace ExcelComponent.Excel
{
    ...
    public class XlsDocument
    {
        ...
        internal XlsDocument(string fileName, Workbook.BytesReadCallback workbookBytesReadCallback)
        {
            ...
            _workbook = new Workbook(this, _ole2Doc.Streams[_ole2Doc.Streams.GetIndex(Directory.Biff8Workbook)].Bytes, workbookBytesReadCallback);
        }

        /// <summary>
        /// Initializes a new XlsDocument from the provided stream, reading in as much information
        /// from the file as possible and representing it appropriately with excel objects
        ...
        public XlsDocument(System.IO.Stream stream)
        {
            ...
            if (stream == null)
                throw new Exception("steam Can't be null" );

            _ole2Doc = new Ole2Document();
            _ole2Doc.Load(stream);

            //TODO: SummaryInformationSection and DocumentSummaryInformationSections should be read by MyOle2
            _workbook = new Workbook(this, _ole2Doc.Streams[_ole2Doc.Streams.GetIndex(Directory.Biff8Workbook)].Bytes, null);
        }
        ...
        /// <summary>
        /// Gets this com.Ole.excel XlsDocument's OLE2 Document.
        /// </summary>
        public Ole2Document OLEDoc { get { return _ole2Doc; } }

        /// <summary>
        /// Gets this com.Ole.excel XlsDocument's Workbook.
        /// </summary>
        ...
        public void Send(SendMethods sendMethod)
        {
            ...
            if (context == null)
                throw new Exception("Current System.Web.HttpContext not found - Send failed.");
