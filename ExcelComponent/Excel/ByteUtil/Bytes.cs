...
namespace ExcelComponent.Excel.ByteUtil
{
    /// <summary>
    ...
    public partial class Bytes
    {
        ...
        internal List<ExcelComponent.Excel.ByteUtil.Bytes> _bytesList = null;
        ...
        public Bytes(ExcelComponent.Excel.ByteUtil.Bytes bytes) : this()
        {
            CheckNewLength(bytes);
            _bytesList = new List<ExcelComponent.Excel.ByteUtil.Bytes>();
        ...
        public void Append(byte[] byteArray)
        {
            ...
            {
                ConvertToList();
                _bytesList.Add(new ExcelComponent.Excel.ByteUtil.Bytes(byteArray));
            }
            else
            {
                _bytesList.Add(new ExcelComponent.Excel.ByteUtil.Bytes(byteArray));
        ...
        public void Append(ExcelComponent.Excel.ByteUtil.Bytes bytes)
        {
            if (bytes.Length == 0)
                return;
        ...
        {
            Prepend(new ExcelComponent.Excel.ByteUtil.Bytes(byteArray));
        }
        ...
        public void Prepend(ExcelComponent.Excel.ByteUtil.Bytes bytes)
        {
            if (bytes.Length == 0)
                return;
        ...
        public ExcelComponent.Excel.ByteUtil.Bytes Get(int getLength)
        {
        ...
        public ExcelComponent.Excel.ByteUtil.Bytes Get(int offset, int getLength)
        {
            ExcelComponent.Excel.ByteUtil.Bytes bytes = new ExcelComponent.Excel.ByteUtil.Bytes();
        ...
        private void Get(int offset, int getLength, ExcelComponent.Excel.ByteUtil.Bytes intoBytes)
        {
            if (offset < 0)
                throw new Exception(string.Format("offset {0} must be >= 0", offset));

            if (getLength < 0)
                throw new Exception(string.Format("getLength {0} must be >= 0", getLength));

            if (offset >= Length)
                throw new Exception(string.Format("offset {0} must be < Length {1}", offset, Length));
            ...
            foreach (ExcelComponent.Excel.ByteUtil.Bytes bytes in _bytesList)
            {
        ...
        internal static byte[] MidByteArray(byte[] byteArray, int offset, int length)
        {
            if (offset >= byteArray.Length)
                throw new Exception(string.Format("offset {0} must be less than byteArray.Length {1}", offset, byteArray.Length));

            if (offset + length > byteArray.Length)
                throw new Exception(string.Format("offset {0} + length {1} must be <= byteArray.Length {2}", offset, length, byteArray.Length));
        ...
        private void CheckNewLength(ExcelComponent.Excel.ByteUtil.Bytes withAddition)
        {
            CheckNewLength(withAddition.Length);
        }
        ...
        private void ConvertToList()
        {
            ...
            ExcelComponent.Excel.ByteUtil.Bytes newBytes = new ExcelComponent.Excel.ByteUtil.Bytes(_byteArray);
            _byteArray = null;
