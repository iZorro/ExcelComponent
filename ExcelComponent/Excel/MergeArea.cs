...
namespace ExcelComponent.Excel
{
    ...
    public struct MergeArea
    {
        ...
        {
            if (rowMin < 1) throw new Exception(  "rowMin must be >= 1");
            if (rowMin > BIFF8.MaxRows) throw new Exception(  "rowMin must be <= " + BIFF8.MaxRows);
            if (rowMax < rowMin) throw new Exception(  "rowMax must be >= rowMin (" + rowMin + ")");
            if (rowMax > BIFF8.MaxRows) throw new Exception(  "rowMax must be <=" + BIFF8.MaxRows);
            if (colMin < 1) throw new Exception( "colMin must be >= 1");
            if (colMin > BIFF8.MaxCols) throw new Exception(  "colMin must be <= " + BIFF8.MaxCols);
            if (colMax < colMin) throw new Exception( "colMax must be >= colMin (" + colMin + ")");
            if (colMax > BIFF8.MaxCols) throw new Exception(  "colMax must be <= " + BIFF8.MaxCols);
